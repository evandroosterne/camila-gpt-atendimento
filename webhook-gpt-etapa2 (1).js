// webhook-gpt-etapa2.js - Compatível com Railway (porta dinâmica)
...
app.post("/webhook", async (req, res) => {
  ...
  const mencionaDor = msg.toLowerCase().includes("dor") || msg.toLowerCase().includes("sangramento") || msg.toLowerCase().includes("urgente");
...
Mensagem: "${msg}"`;
  ...
  try {
    ...
    if (
      msg.toLowerCase().includes("quero agendar") ||
      msg.toLowerCase().includes("marcar consulta") ||
      msg.toLowerCase().includes("falar com atendente")
    ) {
      resposta += `
